...
- [World Stock Exchange account](stockmarket.md) and [Stock API](https://github.com/bitburner-official/bitburner-src/blob/stable/markdown/bitburner.stock.md) Access
...
## Strategy: the reset-loop ("prestige") farming pattern

Because the price multiplier resets every install while reputation-to-favor compounds,
the efficient pattern early in a BitNode is to buy a **few** best-bang-for-buck
Augmentations, install them (a soft reset), and repeat — compounding your multipliers
each cycle. For the full strategy (which augs to buy in what order, the two favor
regimes, faction unlock order, and the automation that runs it), see
[Augmentation Reset-Loop Strategy](../guide/augmentation-strategy.md).
