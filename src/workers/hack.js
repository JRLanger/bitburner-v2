 ...
 *   [4] opTag    — OPTIONAL op label for the telemetry record (e.g. "H").
 ...
 * via ns.writePort — 0 GB, so per-thread RAM is unchanged. `ret` is the money
 * actually stolen (0 = failed hack). Port 6 is HARDCODED (must match
...
export async function main(ns) {
    ...
    const ret = await ns.hack(target, { additionalMsec: delay });
    const expLand = ns.args[3] ?? 0;
    if (expLand > 0) {
        ns.writePort(6, [ns.args[4] ?? "H", target, expLand, Date.now(), ret, ns.args[5] ?? 0]);
    }
}
