 ...
 *  orbiter.js. Network discovery/rooting/provisioning lives in lib/server-provision.js.
...
export async function main(ns) {
    ...
    while (true) {
        ...
        const snap = buildSnapshot(ns, "booster", servers, pool, eligible, batchers, needsPrep, tickGap);
        renderTail(ns, snap);
...
function selectBatchers(ns, eligible, poolTotal) {
    ...
    // PER-BATCH RAM CAP (stall variant 3). No admitted plan may cost more per
    // batch than this — the placeable gate in batchPhase needs one whole batch
    // free at fire time, and the REFILL_HEADROOM invariant only guarantees that
    // while a batch costs less than the floor. On a fresh-BN pool a low-level
    // batch can cost ~80% of the pool: admission fitted it to the BUDGET
    // (a pipeline-total number), the gate then deferred forever at fill=0, and
    // the phantom reservation starved prep too — total bootstrap deadlock
    // (observed: n00dles, 105.65GB/batch vs 100GB poolFree, fill=0/312).
    ...
    for (const t of order) {
        ...
        if (remaining >= baseCost && t.ramPerBatch <= perBatchCap) {
            ...
            const fitted = bestHackPct(ns, t, t.chance, Math.min(remaining, perBatchCap));
            ...
            if (!(wasBatching.has(t.hostname) && withinBand)) {
                ...
                if (t.sec <= t.minSecurity * (1 + SEC_MARGIN)) {
                    ...
                    if (deficit && held < RAMP_DOWN_STABLE_TICKS) {
                        ...
                        ramped = maximizeHackPct(ns, t, t.chance, Math.min(capacity / conc, perBatchCap), t.f);
 ...
 * Grow threads are sized at CURRENT (elevated) security, where growthAnalyze reports
 * a weaker per-thread growth than the min-security the grow actually lands on — an
 * over-provision that clamps harmlessly at max money (on top of THREAD_MARGIN).
 *
...
function prepWave(ns, t, pool, ramBudget = Infinity) {
    ...
    // Cap grow to the remaining budget, leaving a little for the counter-weaken
    // (~1 weaken per 12.5 grow, so the grow share dominates).
